...
export const fetchTasksAction = () => async (dispatch) => {
	...
	try {
		const [a, b] = await Promise.all([
			fetch("https://test.hng.tech/api/user/task", {
				headers: {
		...
		const taskResponse = [].concat.apply([], tasks.data);
...
export const fetchProbationAction = (id) => async (dispatch) => {
	...
	try {
		const a = await fetch(`https://test.hng.tech/api/probation/status/${id}`, {
			headers: {
				Authorization: "Bearer " + localStorage["token"],
			},
		});
...
export const fetchGradeAction = (task_id, user_id) => async (dispatch) => {
	...
	try {
		const a = await fetch(`https://test.hng.tech/api/user/${user_id}/task/${task_id}`, {
			headers: {
				Authorization: "Bearer " + localStorage["token"],
			},
		});
...
export const submitTaskAction = (data) => async (dispatch) => {
	...
	try {
		const a = await fetch(`https://test.hng.tech/api/submit`, {
			method: "POST",
			headers: {
				Authorization: "Bearer " + localStorage["token"],
