...
export const regInAction = (payload) => async (dispatch) => {
	...
	try {
		...
		if (err.response && err.response.status === 401) {
			dispatch(
				regFail(
					"This email or slack username has already been taken. Please make sure they are both unique",
				),
			);
		} else {
...
export const getUserAction = (id) => async (dispatch) => {
	try {
		const profile = await fetch(`https://test.hng.tech/api/user-profile/${id}`, {
			headers: {
